...
class Temporal_Transformer_DoubleCls(nn.Module):
    def __init__(self, num_patches, input_dim, depth, heads, mlp_dim, dim_head):
        super().__init__()
        dropout = 0.0
        self.num_patches = num_patches
        self.input_dim = input_dim
        self.head = nn.Parameter(torch.randn(1, 1, input_dim))
        self.tail = nn.Parameter(torch.randn(1, 1, input_dim))
        self.pos_embedding = nn.Parameter(torch.randn(1, num_patches + 2, input_dim))
        self.temporal_transformer = Transformer(
            input_dim, depth, heads, dim_head, mlp_dim, dropout
        )
    ...
    def forward(self, x):
        b, n, _ = x.shape
        head = repeat(self.head, "() n d -> b n d", b=b)
        tail = repeat(self.tail, "() n d -> b n d", b=b)
        x = torch.cat((head, x, tail), dim=1)
